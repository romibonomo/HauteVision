            ...
            if data.isEmpty {
                ...
                VStack(spacing: 16) {
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text("Add your first measurement to start tracking")
                            .font(.subheadline)
                ...
                HStack(spacing: 0) {
                    VStack(alignment: .trailing, spacing: 0) {
                        let range = FuchsDystrophyView.getValueRange(data)
                ...
                VStack(spacing: 0) {
                    ...
                    ZStack(alignment: .leading) {
                        ...
                        Path { path in
                            ...
                            path.addLine(to: CGPoint(x: horizontalPadding + availableWidth, y: chartHeight + topPadding))
                            path.closeSubpath()
                        }
                        .fill(LinearGradient(
                        ...
                        ForEach(0..<data.count, id: \.self) { index in
                            if index % step == 0 || index == data.count - 1 {
                                ...
                                VStack(spacing: 2) {
                                    ...
                                    Text(Self.formatDateShort(data[index].0))
                                        .font(.caption2)
                                        .foregroundColor(.gray)
                                }
...
struct FuchsMeasurementRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Header with date, time, severity score, and action buttons
            ...
            VStack(spacing: 16) {
                ...
                HStack(spacing: 0) {
                    ...
                    VStack(spacing: 8) {
                        HStack(spacing: 6) {
                            Image(systemName: "eye.circle.fill")
                                .font(.caption)
                                .foregroundColor(.blue)
                            Text(LocalizedStringKey.endothelialCellDensity.localized())
                                .font(.caption)
                                .fontWeight(.medium)
                                .foregroundColor(.secondary)
                        }
                        ...
                        HStack(spacing: 4) {
                            Circle()
                                .fill(ecdStatusColor(measurement.ecd))
                    ...
                    VStack(spacing: 8) {
                        ...
                        HStack(spacing: 4) {
                            Circle()
                                .fill(pachymetryStatusColor(measurement.pachymetry))
                                .frame(width: 8, height: 8)
